...
stats = {
    'Warrior': {'HP': randint(25, 35), 'Atk': randint(19, 21), 'Def': randint(20, 22), 'Spd': randint(1, 7), 'Mana': 30,
                'Subclass': {'Brusier': False, 'Paladin': False}},  # +Atk +Def -HP
    'Mage': {'HP': randint(15, 20), 'Atk': randint(28, 30), 'Def': randint(17, 22), 'Spd': randint(5, 13), 'Mana': 30,
             'Subclass': {'Wizard': False, 'Hemomancer': False}},  # +Atk +Mana -Spd
    'Assassin': {'HP': randint(15, 20), 'Atk': randint(32, 34), 'Def': randint(9, 14), 'Spd': randint(15, 17),
                 'Mana': 30, 'Subclass': {'Ninja': False, 'Rogue': False}},  # +Atk +Spd -HP
    'Marksman': {'HP': randint(20, 25), 'Atk': randint(14, 19), 'Def': randint(11, 19), 'Spd': randint(20, 22),
                 'Mana': 30, 'Subclass': {'Sniper': False, 'Archer': False}},  # +Spd -Def
    'Tank': {'HP': randint(35, 40), 'Atk': randint(1, 8), 'Def': randint(29, 31), 'Spd': randint(1, 6), 'Mana': 30,
             'Subclass': {'Vanguard': False, 'Warden': False}}  # +Def +HP -Atk
}
...
def battle(num, player, pClass, subclass, name, exp, hp, mana, boss):
    ...
    if enemyLevel > 100:
        enemyLevel = 100
    if boss == False:
        enemy = {'Monster': [round((randint(5, 10) + round(randint(1, 2) * (random() * 3) * enemyLevel))),
                             round((randint(7, 11) + round(randint(1, 2) * (random() * 2.5) * enemyLevel))),
                             round((randint(7, 11) + round(randint(1, 2) * (random() * 2.5) * enemyLevel))),
                             round((randint(7, 11) + round(randint(1, 2) * (random() * 2.5) * enemyLevel)))]}
    else:
        enemy = {'Monster': [round((randint(15, 20) + round(randint(3, 4) * (random() * 2.75) * enemyLevel))),
                             round((randint(10, 15) + round(randint(3, 4) * (random() * 2.25) * enemyLevel))),
                             round((randint(10, 15) + round(randint(3, 4) * (random() * 2.25) * enemyLevel))),
                             round((randint(10, 15) + round(randint(3, 4) * (random() * 2.25) * enemyLevel)))]}
    ...
    while enemyAlive == True and Alive == True:
        ...
        if choice == 'attack':
            ...
            if turnOrder(player[name][0]['Spd'], enemy['Monster'][3]) == 1:
                exp, hp, mana, atkBuff, defBuff, spdBuff, enemyHp, enemyAlive = specialAttack(player, subclass, name,
                                                                                              exp, hp, mana, atkBuff,
                                                                                              defBuff, spdBuff, enemy,
                                                                                              enemyHp, boss, enemyLevel,
                                                                                              enAttack)
                if enemyAlive == False:
                    return exp, hp, mana
...
def specialAttack(player, subclass, name, exp, hp, mana, atkBuff, defBuff, spdBuff, enemy, enemyHp, boss, enemyLevel,
                  enAttack):
    alive = True
    if subclass == 'Brusier':
        mana -= 10
        damage = round(((((2 * player[name][1] / 5 + 2) * player[name][0]['Atk'] * randint(150, 200) / enemy['Monster'][
            2]) / 50) + 2) * (randint(1, 100) / 100))
        enemyHp -= damage
        ...
        if mana >= 20:
            ...
            damage = round(((((2 * player[name][1] / 5 + 2) * player[name][0]['Atk'] * randint(30, 100) /
                              enemy['Monster'][2]) / 50) + 2) * (randint(1, 100) / 100))
            if enemyHp - damage < 0:
    ...
    while exp > lvlup:
        ...
        if player[name][1] < 100:
            if exp > lvlup:
                ...
                print('HP: ' + str(player[name][0]['HP']) + ' -> ' + str(player[name][0]['HP'] + hpGain) + ' +' + str(
                    hpGain))
                print('Attack: ' + str(player[name][0]['Atk']) + ' -> ' + str(
                    player[name][0]['Atk'] + atkGain) + ' +' + str(atkGain))
                print('Defense: ' + str(player[name][0]['Def']) + ' -> ' + str(
                    player[name][0]['Def'] + defGain) + ' +' + str(defGain))
                print('Speed: ' + str(player[name][0]['Spd']) + ' -> ' + str(
                    player[name][0]['Spd'] + spdGain) + ' +' + str(spdGain))
                print('Mana: ' + str(player[name][0]['Mana']) + ' -> ' + str(
                    player[name][0]['Mana'] + manaGain) + ' +' + str(manaGain))
                print('\n')
